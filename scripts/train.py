...
def calc_metrics(y_true, y_pred):
    values = {"rmlse": rmlse(y_true, y_pred)}
    return values
...
def main():
    ...
    password_model.fit(train_data=train, test_data=val.drop(columns=[target]), **params)
    os.mkdir("model")
    ...
    password_model_loaded = PasswordLSTM(
        config=config, model_serialized=model_path, tokenizer=tokenizer_path
    )
    ...
    password_model = PasswordLSTM(config=config)
    password_model.fit(
        train_data=pd.concat([train, val]).reset_index(drop=True),
